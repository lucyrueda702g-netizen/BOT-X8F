...
// ============================================================
// VIP MEMBERS SYSTEM
// Publica el servidor del VIP en otros servidores para ganar miembros
// ============================================================

const vipMembers    = new Map(); // userId -> {invite, description, tag, since, active}
const vipChannel    = new Map(); // guildId -> channelId (canal donde se publican VIPs)
const vipRequests   = new Map(); // userId -> {invite, description, tag, status}
const vipAdChannel  = new Map(); // guildId -> channelId (canal donde se anuncian ads)
const vipBoosted    = new Map(); // userId -> timestamp last boost

// VIP COMMANDS (agregar a commands array arriba también)
const vipCommands = [
    new SlashCommandBuilder()
        .setName('vip')
        .setDescription('👑 Ver tu estado VIP y beneficios'),

    new SlashCommandBuilder()
        .setName('vip-solicitar')
        .setDescription('📨 Solicitar membresía VIP para publicar tu servidor')
        .addStringOption(o => o.setName('invite').setDescription('Link de invitación de tu servidor (discord.gg/...)').setRequired(true))
        .addStringOption(o => o.setName('descripcion').setDescription('Descripción de tu servidor (qué ofrece)').setRequired(true))
        .addStringOption(o => o.setName('tag').setDescription('Categoría: Gaming, Anime, Roblox, etc.').setRequired(true)),

    new SlashCommandBuilder()
        .setName('vip-aprobar')
        .setDescription('✅ Aprobar solicitud VIP de un usuario')
        .addUserOption(o => o.setName('usuario').setDescription('Usuario a aprobar').setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    new SlashCommandBuilder()
        .setName('vip-rechazar')
        .setDescription('❌ Rechazar solicitud VIP')
        .addUserOption(o => o.setName('usuario').setDescription('Usuario').setRequired(true))
        .addStringOption(o => o.setName('razon').setDescription('Razón del rechazo'))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    new SlashCommandBuilder()
        .setName('vip-revocar')
        .setDescription('🚫 Revocar VIP de un usuario')
        .addUserOption(o => o.setName('usuario').setDescription('Usuario').setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    new SlashCommandBuilder()
        .setName('vip-lista')
        .setDescription('📋 Ver todos los miembros VIP activos'),

    new SlashCommandBuilder()
        .setName('vip-solicitudes')
        .setDescription('📬 Ver solicitudes VIP pendientes')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    new SlashCommandBuilder()
        .setName('vip-publicar')
        .setDescription('📢 Publicar tu servidor en todos los canales de anuncios VIP')
        .addStringOption(o => o.setName('mensaje').setDescription('Mensaje extra para el anuncio (opcional)')),

    new SlashCommandBuilder()
        .setName('vip-canal')
        .setDescription('📌 Configurar canal donde se muestran los VIPs')
        .addChannelOption(o => o.setName('canal').setDescription('Canal VIP').setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    new SlashCommandBuilder()
        .setName('vip-ads-canal')
        .setDescription('📢 Configurar canal donde se reciben anuncios VIP de otros servidores')
        .addChannelOption(o => o.setName('canal').setDescription('Canal de anuncios').setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    new SlashCommandBuilder()
        .setName('vip-editar')
        .setDescription('✏️ Editar tu información VIP')
        .addStringOption(o => o.setName('descripcion').setDescription('Nueva descripción'))
        .addStringOption(o => o.setName('invite').setDescription('Nuevo link de invitación'))
        .addStringOption(o => o.setName('tag').setDescription('Nueva categoría')),

    new SlashCommandBuilder()
        .setName('vip-boost')
        .setDescription('🚀 Boost: destacar tu anuncio VIP por 24h'),

].map(c => c.toJSON());

// Register VIP commands globally
;(async () => {
    const rest2 = new REST({version:'10'}).setToken(process.env.TOKEN);
    client.once('ready', async () => {
        try {
            const existing = await rest2.get(Routes.applicationCommands(client.user.id));
            const allCmds = [...commands, ...vipCommands];
            await rest2.put(Routes.applicationCommands(client.user.id), {body: allCmds});
            console.log('✅ VIP commands registered!');
        } catch(e) { console.error('VIP cmd error:', e.message); }
    });
})();

// ── VIP EMBED BUILDER ────────────────────────────────────────
function buildVIPEmbed(userId, vip, boosted) {
    const tagColors = {
        Gaming: 0x5865F2, Anime: 0xFF69B4, Roblox: 0xED4245,
        General: 0x57F287, Arte: 0xFF8C00, Música: 0x9B59B6,
        Programación: 0x00BCD4, default: 0xFFD700,
    };
    const color = tagColors[vip.tag] || tagColors.default;

    const embed = new EmbedBuilder()
        .setTitle((boosted ? '🚀 ' : '👑 ') + '[VIP] ' + (vip.serverName || 'Servidor de Discord'))
        .setDescription(vip.description)
        .setColor(boosted ? 0xFFD700 : color)
        .addFields(
            { name: '🔗 Únete aquí', value: vip.invite, inline: true },
            { name: '🏷️ Categoría', value: vip.tag, inline: true },
            { name: '👤 Publicado por', value: `<@${userId}>`, inline: true },
        )
        .setFooter({ text: boosted ? '⭐ ANUNCIO DESTACADO • VIP Member' : '👑 VIP Member • Haz /vip-publicar para anunciar tu servidor' })
        .setTimestamp();

    return embed;
}

// ── VIP INTERACTION HANDLER ───────────────────────────────────
client.on('interactionCreate', async interaction => {
    if (!interaction.isChatInputCommand()) return;
    const { commandName: cmd, guild, member, user } = interaction;
    ...
    // ── VIP STATUS ──
    if (cmd === 'vip') {
        const vip = vipMembers.get(user.id);
        const req = vipRequests.get(user.id);

        if (!vip && !req) {
            const embed = new EmbedBuilder()
                .setTitle('👑 VIP Members')
                .setDescription('¡Conviértete en VIP y **publica tu servidor** en cientos de discords para conseguir más miembros!')
                .setColor(0xFFD700)
                .addFields(
                    { name: '✨ Beneficios VIP', value: '🔹 Publicar tu servidor automáticamente\n🔹 Tu anuncio llega a TODOS los servidores con el bot\n🔹 Más miembros para tu comunidad\n🔹 Anuncio con diseño profesional\n🔹 Boost para destacar tu anuncio\n🔹 Badge VIP en tu perfil', inline: false },
                    { name: '📨 Cómo obtenerlo', value: 'Usa `/vip-solicitar` con el link de tu servidor y una descripción', inline: false },
                )
                .setFooter({ text: 'Sistema VIP | MXYKELxELIK Bot' });
            return interaction.reply({ embeds: [embed] });
        }

        if (req && !vip) {
            const embed = new EmbedBuilder()
                .setTitle('⏳ Solicitud VIP Pendiente')
                .setDescription('Tu solicitud está siendo revisada por los administradores')
                .setColor(0xFEE75C)
                .addFields(
                    { name: '🔗 Invite', value: req.invite, inline: true },
                    { name: '🏷️ Tag', value: req.tag, inline: true },
                    { name: '📝 Descripción', value: req.description, inline: false },
                )
        ...
        if (vip) {
            const isBoosted = vipBoosted.has(user.id) && (Date.now() - vipBoosted.get(user.id)) < 86400000;
            const embed = new EmbedBuilder()
                .setTitle('👑 Tu Estado VIP')
                .setColor(0xFFD700)
                .addFields(
                    { name: '✅ Estado', value: '**ACTIVO**', inline: true },
                    { name: '🔗 Invite', value: vip.invite, inline: true },
                    { name: '🏷️ Tag', value: vip.tag, inline: true },
                    { name: '📝 Descripción', value: vip.description, inline: false },
                    { name: '🚀 Boost', value: isBoosted ? '⭐ ACTIVO (destacado)' : 'Sin boost — usa `/vip-boost`', inline: true },
                    { name: '📢 Publicar', value: 'Usa `/vip-publicar` para anunciar tu servidor ahora', inline: false },
                )
                .setFooter({ text: 'VIP desde ' + new Date(vip.since).toLocaleDateString() });
            return interaction.reply({ embeds: [embed], ephemeral: true });
        }
    }

    // ── VIP SOLICITAR ──
    else if (cmd === 'vip-solicitar') {
        if (vipMembers.has(user.id)) return interaction.reply({ content: '✅ Ya eres VIP! Usa `/vip` para ver tu estado', ephemeral: true });
        if (vipRequests.has(user.id)) return interaction.reply({ content: '⏳ Ya tienes una solicitud pendiente', ephemeral: true });
        ...
        if (!invite.includes('discord.gg/') && !invite.includes('discord.com/invite/')) {
            return interaction.reply({ content: '❌ El link debe ser un invite de Discord (discord.gg/...)', ephemeral: true });
        }

        vipRequests.set(user.id, { invite, description, tag, since: Date.now(), status: 'pending' });

        // Notify admins
        const embed = new EmbedBuilder()
            .setTitle('📬 Nueva Solicitud VIP')
            .setColor(0xFEE75C)
            .addFields(
            ...
            .setFooter({ text: 'Usa /vip-aprobar @usuario o /vip-rechazar @usuario' })
            .setTimestamp();

        // Send to admins (logs channel)
        const config = getConfig(guild.id);
        if (config.logsChannel) {
            const ch = guild.channels.cache.get(config.logsChannel);
            if (ch) await ch.send({ content: '📬 **Nueva solicitud VIP!** — Revisa con `/vip-solicitudes`', embeds: [embed] });
        }

        await interaction.reply({
            embeds: [new EmbedBuilder()
                .setTitle('✅ Solicitud enviada!')
                .setDescription('Tu solicitud VIP fue enviada. Los administradores la revisarán pronto.\n\nCuando sea aprobada, usa `/vip-publicar` para anunciar tu servidor en todos los discords con el bot!')
                .setColor(0x57F287)],
            ephemeral: true
        });
    }
    ...
    // ── VIP APROBAR ──
    else if (cmd === 'vip-aprobar') {
        const target = interaction.options.getUser('usuario');
        const req = vipRequests.get(target.id);
        if (!req) return interaction.reply({ content: '❌ Este usuario no tiene solicitud pendiente', ephemeral: true });

        // Promote to VIP
        vipMembers.set(target.id, {
            invite: req.invite,
        ...
        vipRequests.delete(target.id);

        // Notify user
        await target.send({
            embeds: [new EmbedBuilder()
                .setTitle('🎉 ¡Felicidades! Eres VIP')
                .setDescription('Tu solicitud VIP fue **aprobada**!\n\nAhora puedes usar `/vip-publicar` para anunciar tu servidor en todos los discords que tienen el bot.')
                .setColor(0xFFD700)
                .addFields({ name: '🔗 Tu invite', value: req.invite }, { name: '📢 Próximo paso', value: 'Usa `/vip-publicar` para empezar!' })]
        }).catch(() => {});

        // Give VIP role if exists
        const vipRole = guild.roles.cache.find(r => r.name.toLowerCase().includes('vip'));
        if (vipRole) {
            const gm = await guild.members.fetch(target.id).catch(() => null);
            if (gm) await gm.roles.add(vipRole).catch(() => {});
        }

        await interaction.reply({
            embeds: [new EmbedBuilder()
                .setDescription('✅ **' + target.tag + '** ahora es VIP! Se le notificó por DM.')
                .setColor(0x57F287)]
        });

        logAction(guild, '👑 Nuevo VIP', user.tag + ' aprobó a ' + target.tag + ' como VIP', 0xFFD700);
    }

    // ── VIP RECHAZAR ──
    else if (cmd === 'vip-rechazar') {
        const target = interaction.options.getUser('usuario');
        ...
        await target.send({
            embeds: [new EmbedBuilder()
                .setTitle('❌ Solicitud VIP rechazada')
                .setDescription('Tu solicitud VIP fue rechazada.\n**Razón:** ' + razon + '\n\nPuedes volver a solicitar con `/vip-solicitar`.')
                .setColor(0xED4245)]
        }).catch(() => {});

        await interaction.reply({ embeds: [new EmbedBuilder().setDescription('✅ Solicitud de **' + target.tag + '** rechazada').setColor(0xED4245)] });
    }

    // ── VIP REVOCAR ──
    else if (cmd === 'vip-revocar') {
        const target = interaction.options.getUser('usuario');
        if (!vipMembers.has(target.id)) return interaction.reply({ content: '❌ Este usuario no es VIP', ephemeral: true });
        vipMembers.delete(target.id);

        await target.send({
            embeds: [new EmbedBuilder()
                .setTitle('🚫 VIP Revocado')
                .setDescription('Tu membresía VIP fue revocada por los administradores.')
                .setColor(0xED4245)]
        }).catch(() => {});

        await interaction.reply({ embeds: [new EmbedBuilder().setDescription('✅ VIP de **' + target.tag + '** revocado').setColor(0xED4245)] });
    }

    // ── VIP LISTA ──
    else if (cmd === 'vip-lista') {
        const vips = [...vipMembers.entries()];
        if (vips.length === 0) return interaction.reply({ content: '❌ No hay miembros VIP aún', ephemeral: true });

        const embed = new EmbedBuilder()
            .setTitle('👑 Miembros VIP (' + vips.length + ')')
            .setColor(0xFFD700)
            .setDescription(vips.map(([uid, v], i) =>
                '**' + (i+1) + '.** <@' + uid + '> — `' + v.tag + '`\n🔗 ' + v.invite + '\n*' + v.description.slice(0, 60) + '...*'
            ).slice(0, 10).join('\n\n'))
            .setFooter({ text: 'Usa /vip-solicitar para unirte' })
            .setTimestamp();
    ...
    // ── VIP SOLICITUDES ──
    else if (cmd === 'vip-solicitudes') {
        const reqs = [...vipRequests.entries()];
        if (reqs.length === 0) return interaction.reply({ content: '✅ No hay solicitudes pendientes', ephemeral: true });

        const embed = new EmbedBuilder()
            .setTitle('📬 Solicitudes VIP Pendientes (' + reqs.length + ')')
            .setColor(0xFEE75C)
            .setDescription(reqs.map(([uid, r], i) =>
    ...
    // ── VIP PUBLICAR ──
    else if (cmd === 'vip-publicar') {
        const vip = vipMembers.get(user.id);
        if (!vip) return interaction.reply({ content: '❌ No eres VIP. Usa `/vip-solicitar` para solicitarlo', ephemeral: true });

        const extraMsg = interaction.options.getString('mensaje') || '';
        const isBoosted = vipBoosted.has(user.id) && (Date.now() - vipBoosted.get(user.id)) < 86400000;

        const embed = buildVIPEmbed(user.id, vip, isBoosted);
        if (extraMsg) embed.addFields({ name: '💬 Mensaje del dueño', value: extraMsg });

        // Publish to ALL guilds that have vip-ads-canal configured
        let sentCount = 0;
        for (const [guildId, channelId] of vipAdChannel.entries()) {
            const targetGuild = client.guilds.cache.get(guildId);
            if (!targetGuild) continue;
            const targetCh = targetGuild.channels.cache.get(channelId);
            if (!targetCh) continue;

            await targetCh.send({
                content: isBoosted ? '⭐ **ANUNCIO VIP DESTACADO**' : '👑 **ANUNCIO VIP**',
                embeds: [embed]
            }).then(() => sentCount++).catch(() => {});
        }

        // Also publish in current guild's vip channel
        const config = getConfig(guild.id);
        if (config.welcomeChannel) {
            const localVipCh = vipChannel.get(guild.id);
            if (localVipCh) {
                const ch = guild.channels.cache.get(localVipCh);
        ...
        await interaction.reply({
            embeds: [new EmbedBuilder()
                ...
                .setDescription('Tu servidor fue anunciado en **' + sentCount + ' servidores** con el bot!\n\n🔗 **' + vip.invite + '**\n\nPronto empezarás a recibir nuevos miembros 🚀')
                .setColor(0x57F287)
                .addFields({ name: '💡 Consejo', value: 'Usa `/vip-boost` para destacar tu anuncio y llegar a más personas!' })],
            ephemeral: true
        });

        logAction(guild, '📢 VIP Publicó', user.tag + ' publicó su servidor: ' + vip.invite, 0xFFD700);
    }

    // ── VIP CANAL ──
    else if (cmd === 'vip-canal') {
        const ch = interaction.options.getChannel('canal');
        vipChannel.set(guild.id, ch.id);
        await interaction.reply({ embeds: [new EmbedBuilder().setDescription('✅ Canal VIP → ' + ch + '\nAquí se mostrarán todos los miembros VIP').setColor(0x57F287)] });
    }

    // ── VIP ADS CANAL ──
    else if (cmd === 'vip-ads-canal') {
        const ch = interaction.options.getChannel('canal');
        vipAdChannel.set(guild.id, ch.id);

        // Post current VIPs immediately
        let posted = 0;
        for (const [uid, vip] of vipMembers.entries()) {
            const embed = buildVIPEmbed(uid, vip, false);
            await ch.send({ content: '👑 **ANUNCIO VIP**', embeds: [embed] }).then(() => posted++).catch(() => {});
        }

        await interaction.reply({
            embeds: [new EmbedBuilder()
                .setDescription('✅ Canal de anuncios VIP → ' + ch + '\nAquí recibirás anuncios de servidores VIP de toda la red!\n' + (posted > 0 ? posted + ' anuncios existentes publicados.' : ''))
                .setColor(0x57F287)]
        });
    }

    // ── VIP EDITAR ──
    else if (cmd === 'vip-editar') {
        const vip = vipMembers.get(user.id);
        if (!vip) return interaction.reply({ content: '❌ No eres VIP', ephemeral: true });

        const newDesc = interaction.options.getString('descripcion');
        const newInvite = interaction.options.getString('invite');
        const newTag = interaction.options.getString('tag');

        if (newDesc) vip.description = newDesc;
        if (newInvite) vip.invite = newInvite;
        if (newTag) vip.tag = newTag;

        vipMembers.set(user.id, vip);

        await interaction.reply({
            embeds: [new EmbedBuilder()
                .setTitle('✅ Info VIP actualizada')
                .addFields(
                    { name: '🔗 Invite', value: vip.invite, inline: true },
                    { name: '🏷️ Tag', value: vip.tag, inline: true },
                    { name: '📝 Descripción', value: vip.description, inline: false },
    ...
    // ── VIP BOOST ──
    else if (cmd === 'vip-boost') {
        const vip = vipMembers.get(user.id);
        if (!vip) return interaction.reply({ content: '❌ No eres VIP', ephemeral: true });

        const lastBoost = vipBoosted.get(user.id) || 0;
        const cooldown = 86400000; // 24h
        if (Date.now() - lastBoost < cooldown) {
            const remaining = Math.ceil((cooldown - (Date.now() - lastBoost)) / 3600000);
            return interaction.reply({ content: '⏰ Ya usaste boost hoy. Próximo boost en **' + remaining + 'h**', ephemeral: true });
        }

        vipBoosted.set(user.id, Date.now());

        // Send boosted announcement to all servers
        const embed = buildVIPEmbed(user.id, vip, true);
        let sentCount = 0;
        for (const [guildId, channelId] of vipAdChannel.entries()) {
            ...
            if (!ch) continue;
            await ch.send({ content: '⭐ **¡ANUNCIO VIP DESTACADO!** ⭐', embeds: [embed] }).then(() => sentCount++).catch(() => {});
        }

        await interaction.reply({
            embeds: [new EmbedBuilder()
                .setTitle('🚀 ¡BOOST ACTIVADO!')
                .setDescription('Tu anuncio fue **destacado** en **' + sentCount + ' servidores**!\n\nTu servidor aparece primero y con diseño especial dorado ⭐\n\nPróximo boost disponible en **24 horas**.')
                .setColor(0xFFD700)],
            ephemeral: true
        });
    }
        ...
        console.error('VIP Error en ' + cmd + ':', e.message);
        const m = { content: '❌ Error: ' + e.message, ephemeral: true };
        if (interaction.deferred) await interaction.editReply(m).catch(() => {});
        else if (!interaction.replied) await interaction.reply(m).catch(() => {});
    }
